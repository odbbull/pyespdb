...
def getConnection(pConn):
   try:
      pConn = mysql.connector.connect(user='root', password='welcome1',
...
def genQueryText(pDbId, pMetric, pAcronym, pInstance):
   ...
   pQuery = "select mp_plotdate pd1, cast(mp_plotvalue as decimal(10,2)) pv1 from sp_metricplot    \
              ...
              "' and mp_instance = 1 and sp_database_db_id = " + pDbId + ";"
   
   pQuery1 = "select mp_plotdate pd2, cast(mp_plotvalue as decimal(10,2)) pv2 from sp_metricplot    \
              where mp_metricName = '" + pMetric + "' and mp_metricAcronym = '" + pAcronym +           \
              "' and mp_instance = 2 and sp_database_db_id = " + pDbId + ";"
...
if (__name__ == '__main__'):
    ...
    vQuery, vQuery1, hdrQuery = genQueryText(vDbId, vMetric, vAcronym, vInstance)
    ...
    if vName == 'CPU':
       vTitleName = 'Sessions on CPU for ' + vName
    else:
       vTitleName = vMetric + ' Details of ' + vName
