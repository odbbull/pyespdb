...
def genQueryText(pDbId, pMetric, pAcronym, pInstance, pAddInst):


   pQuery = (f"select mp_plotdate pd1, cast(mp_plotvalue as decimal(10,2)) pv1 from sp_metricplot    \
              where mp_metricName = '{ pMetric }' and mp_metricAcronym = '{ pAcronym }'                  \
              and mp_instance = '{ pInstance }' and sp_database_db_id = '{ pDbId }';")
   
   pQuery1 = (f"select mp_plotdate pd2, cast(mp_plotvalue as decimal(10,2)) pv2 from sp_metricplot    \
              where mp_metricName = '{ pMetric }' and mp_metricAcronym = '{ pAcronym }'          \
              and mp_instance = '{ pAddInst }' and sp_database_db_id = '{ pDbId }';")

   hdrQuery = (f"select db_id, db_name from sp_database where db_id = '{ pDbId }';")

   return pQuery, pQuery1, hdrQuery
...
if (__name__ == '__main__'):
    ...
    ArgParser.add_option("-i",         dest="vInstance",   default=1,    type=int,   help="Instance ID")
    ...
    if not vInstance:
       vInstance = 1
    vAddInst = vInstance + 1

    acmeConn = getConnection(acmeConn)
    
    vQuery, vQuery1, hdrQuery = genQueryText(vDbId, vMetric, vAcronym, vInstance, vAddInst)
    
    cpuPlot1 = pd.read_sql_query(vQuery, acmeConn, parse_dates=['pd1'])
    cpuPlot2 = pd.read_sql_query(vQuery1, acmeConn, parse_dates=['pd2'])

    vId, vName = getDatabaseName(acmeConn, hdrQuery)
    if vName == 'CPU':
       vTitleName = (f"Sessions on CPU for { vId }: { vName }")
    else:
       vTitleName = (f"{ vMetric } details of { vId }: { vName }")

# fig = px.scatter(cpuPlot1, x='pd1', y='pv1', size='pv1', title="Throughput MBPS for DB: 37O")
    fig = px.scatter(title=vTitleName)

# fig = go.Figure()

    fig.add_trace(go.Scatter(x=cpuPlot1.pd1, y=cpuPlot1.pv1, mode='lines', name=vAcronym, line_color='blue'))
    fig.add_trace(go.Scatter(x=cpuPlot2.pd2, y=cpuPlot2.pv2, mode='lines', name=vAcronym, line_color='red'))
...
# fig.update_traces(marker_line_width=2, marker_size=10)
    ...
    fig.update_yaxes(title=vTitleName)
