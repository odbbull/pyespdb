#!/usr/bin/env python3
"""
migrate_sp_metricplot.py
=======================
Migrates sp_metricplot (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - mp_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_metricplot.py
"""
...
MYSQL_COLS = [
    "mp_id",           # used for ordering only, not inserted
    "mp_metricname",
    "mp_metricacronym",
    "mp_instance",
    "mp_plotdate",
...
# Columns to INSERT into PostgreSQL (mp_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "mp_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY mp_id")
