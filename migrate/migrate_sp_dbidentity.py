#!/usr/bin/env python3
"""
migrate_sp_dbidentity.py
=======================
Migrates sp_dbidentity (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - iden_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_dbidentity.py
"""
...
MYSQL_COLS = [
    "iden_id",           # used for ordering only, not inserted
    "iden_metric",
    "iden_acronym",
    "iden_instance",
    "iden_metricdate",
...
# Columns to INSERT into PostgreSQL (iden_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "iden_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY iden_id")
