#!/usr/bin/env python3
"""
migrate_sp_client.py
=======================
Migrates sp_client (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - cl_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
MYSQL_COLS = [
    "cl_id",           # used for ordering only, not inserted
    "cl_name",
    "cl_shortname",
    "cl_creation_date",
    "is_active_fg",
...
# Columns to INSERT into PostgreSQL (cl_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "cl_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY cl_id")
