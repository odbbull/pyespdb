#!/usr/bin/env python3
"""
migrate_sp_exaframe.py
=======================
Migrates sp_exaframe (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - fr_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_exaframe.py
"""
...
MYSQL_COLS = [
    "fr_id",           # used for ordering only, not inserted
    "fr_name",
    "fr_location",
    "fr_model",
    "is_active_fg"
...
# Columns to INSERT into PostgreSQL (fr_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "fr_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY fr_id")
