#!/usr/bin/env python3
"""
migrate_sp_database.py
=======================
Migrates sp_database (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - db_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_database.py
"""
...
MYSQL_COLS = [
    "db_id",           # used for ordering only, not inserted
    "db_name",
    "db_shortname",
    "db_filename",
    "db_fileread_date",
...
# Columns to INSERT into PostgreSQL (db_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS ]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY db_id")
        ...
        insert_sql = (
            ...
            f'OVERRIDING SYSTEM VALUE VALUES ({", ".join(["%s"] * len(PG_INSERT_COLS))})'
        )
