#!/usr/bin/env python3
"""
migrate_sp_category.py
=======================
Migrates sp_category (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - cat_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_category.py
"""
...
MYSQL_COLS = [
    "cat_id",           # used for ordering only, not inserted
    "cat_name",
    "cat_acronym",
    "is_static_fg",
    "is_active_fg",
...
# Columns to INSERT into PostgreSQL (cat_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "cat_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY cat_id")
