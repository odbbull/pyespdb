#!/usr/bin/env python3
"""
migrate_sp_collection.py
=======================
Migrates sp_collection (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - coll_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_collection.py
"""
...
MYSQL_COLS = [
    "coll_id",           # used for ordering only, not inserted
    "coll_name",
    "coll_shortname",
    "coll_date",
    "is_active_fg",
...
# Columns to INSERT into PostgreSQL (coll_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "coll_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY coll_id")
