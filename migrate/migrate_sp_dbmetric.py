#!/usr/bin/env python3
"""
migrate_sp_dbmetric.py
=======================
Migrates sp_dbmetric (43 rows) from MySQL AcmeAnvil -> PostgreSQL pyespdb.

Schema notes:
  - metr_id is excluded from INSERT (PG uses GENERATED ALWAYS AS IDENTITY)
...
Usage:
    python3 migrate_sp_dbmetric.py
"""
...
MYSQL_COLS = [
    "metr_id",           # used for ordering only, not inserted
    "metr_metric",
    "metr_acronym",
    "metr_instance",
    "metr_metricdate",
...
# Columns to INSERT into PostgreSQL (metr_id excluded — GENERATED ALWAYS AS IDENTITY)
PG_INSERT_COLS = [c for c in MYSQL_COLS if c != "metr_id"]
...
def main():
    ...
    try:
        ...
        mysql_cur.execute(f"SELECT {select_cols} FROM `{TABLE}` ORDER BY metr_id")
